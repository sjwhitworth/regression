...
//Saves an initialised model as a gob, for loading later.
func (r *Regression) Save(filename string) {
	if !r.Initialised {
		fmt.Println("Error: You have to initialise the model before you can save it.")
		return
	}

	regbuffer := new(bytes.Buffer)
	encoder := gob.NewEncoder(regbuffer)
	encoder.Encode(r)

	err := ioutil.WriteFile(filename, regbuffer.Bytes(), 0600)
	if err != nil {
		panic(err)
	}
}

//Loads a saved model and returns a pointer to it
func Load(filename string) *Regression {
